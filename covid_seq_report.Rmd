...
```{r, transform-data, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, subset-data, echo=FALSE, message=FALSE, warning=FALSE}
df <- subset(df_assemblies, geo_state == params$state)
df_good <- subset(df, genome_status != "failed_sequencing")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
data.frame(date=seq(as.Date("2020/1/1"), Sys.Date(), by="day")) %>% 
    mutate(week=isoweek(date),year=year(date)) %>% 
    ddply(.(year,week), mutate, epiweek_end=max(date)) ->
    week_calendar
df_dates <- merge(df_good, week_calendar, by.x='collection_date', by.y='date')
ggplot(df_dates) + geom_histogram(aes(epiweek_end, fill=nextclade_clade)) +
