...
Generated for \textbf{`r params$state`} on \textbf{`r paste(Sys.Date())`}}

\fancyfoot[L]{Created on \textbf{`r paste(Sys.Date())`}.}
...
# fix dates
df_assemblies$run_date <- as.Date(df_assemblies$run_date, tryFormats="%Y-%m-%d")
df_assemblies$collection_date <- as.Date(df_assemblies$collection_date, tryFormats="%Y-%m-%d")
...
# CDC epiweeks
data.frame(date=seq(as.Date("2020-01-01", tryFormats="%Y-%m-%d"), Sys.Date()+7, by="day")) %>%
...
This reports summarizes samples recent sequencing activity in the state of **`r params$state`** as of **`r params$date`**.
...
df_run_weeks %>%
  ...
  subset(epiweek_end >= as.Date("2021-01-01", tryFormats="%Y-%m-%d")) %>%
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(df_assemblies, height=4, width=6, units='in') + geom_point(aes(
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(df_good, height=4, width=6, units='in') + geom_point(aes(
