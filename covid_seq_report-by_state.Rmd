...
df_run_weeks %>%
  ...
  df_latest_week
df_latest_week %>%
  ddply(.(genome_status), summarise, n=length(assembly_fasta)) %>%
...
snapshot <- data.frame(
  ...
  cumulative = c(sum(seq_by_week$samples_sequenced), sum(seq_by_week$genomes_assembled), sum(seq_by_week$genomes_submittable), length(subset(df_run_weeks, is_voc)$assembly_fasta)))
```
...
This report, generated on `r params$date`, summarizes our sequencing activity for patient samples collected in **`r params$state`**. It is current as of the epiweek ending **`r latest_epiweek`**.
